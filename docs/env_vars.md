...
These flags control whether OpenAPI `$ref` entries are resolved outside of the spec file. Local file references are enabled by default, matching the JVM engine's historical behaviour. Remote (http/https) references are disabled by default and must be opted into explicitly, so mocks do not make unexpected outbound HTTP calls in locked-down environments.
...
### TLS (h2)
```bash
export IMPOSTER_TLS_CERT_FILE=/etc/imposter/tls/server.crt
export IMPOSTER_TLS_KEY_FILE=/etc/imposter/tls/server.key
imposter
```
