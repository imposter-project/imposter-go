...
func ProcessInterceptor(rs *response.ResponseState, r *http.Request, body []byte, interceptor config.Interceptor, requestStore store.Store, imposterConfig *config.ImposterConfig, processor response.Processor) bool {
	...
	if interceptor.Response != nil {
		processor(rs, r, *interceptor.Response, requestStore)
	}
