...
func TestProcessResponseScriptPrecedence(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "configured headers override script headers of the same name",
			setupState:     func(rs *exchange.ResponseState) { rs.Headers["X-Shared"] = "script" },
			response:       config.Response{Headers: map[string]string{"X-Shared": "config"}},
			expectedStatus: http.StatusOK,
			expectedHeader: map[string]string{"X-Shared": "config"},
		},
		{
			name:           "script headers are retained when configuration does not set them",
			setupState:     func(rs *exchange.ResponseState) { rs.Headers["X-Script"] = "script" },
